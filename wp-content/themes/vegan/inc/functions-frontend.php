...
if ( ! function_exists( 'vegan_post_tags' ) ) {
	function vegan_post_tags() {
		$posttags = get_the_tags();
		if ( $posttags ) {
			echo '<span class="entry-tags-list"><strong>'.esc_html__( 'Tags: ' , 'vegan' ).'</strong> ';
			$i = 1;
			$size = count( $posttags );
...
if ( ! function_exists( 'vegan_post_format_link_helper' ) ) {
	function vegan_post_format_link_helper( $content = null, $title = null, $post = null ) {
		if ( ! $content ) {
			$post = get_post( $post );
			$title = $post->post_title;
			$content = $post->post_content;
		}
		$link = vegan_get_first_url_from_string( $content );
		if ( ! empty( $link ) ) {
...
if ( !function_exists('vegan_get_page_title') ) {
	function vegan_get_page_title() {
		$title = '';
		if ( !is_front_page() || is_paged() ) {
			...
			if ( is_home() ) {
				$title = esc_html__( 'The Blogs', 'vegan' );
			} elseif (is_category()) {
				global $wp_query;
				...
				if ( get_post_type() != 'post' ) {
					...
					$title = esc_html__( 'The Blog', 'vegan' );
				}
				...
				$title = esc_html__('Search results for "','vegan')  . get_search_query();
			} elseif ( is_tag() ) {
				$title = esc_html__('Posts tagged "', 'vegan'). single_tag_title('', false) . '"';
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				$title = esc_html__('Articles posted by ', 'vegan') . $userdata->display_name;
			} elseif ( is_404() ) {
				$title = esc_html__('Error 404', 'vegan');
			}
		}
		return $title;
	}
}
if ( ! function_exists( 'vegan_breadcrumbs' ) ) {
	function vegan_breadcrumbs() {

		$delimiter = ' ';
		$home = esc_html__('Home', 'vegan');
		...
		if ( !is_front_page() || is_paged()) {
			...
			if (is_category()) {
				...
				echo trim($before) . esc_html__('Search results for "','vegan')  . get_search_query() . '"' . $after;
				$title = esc_html__('Search results for "','vegan')  . get_search_query();
			} elseif ( is_tag() ) {
				echo trim($before) . esc_html__('Posts tagged "', 'vegan'). single_tag_title('', false) . '"' . $after;
				$title = esc_html__('Posts tagged "', 'vegan'). single_tag_title('', false) . '"';
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo trim($before) . esc_html__('Articles posted by ', 'vegan') . $userdata->display_name . $after;
				$title = esc_html__('Articles posted by ', 'vegan') . $userdata->display_name;
			} elseif ( is_404() ) {
				echo trim($before) . esc_html__('Error 404', 'vegan') . $after;
				$title = esc_html__('Error 404', 'vegan');
			}
...
if ( ! function_exists( 'vegan_render_breadcrumbs' ) ) {
	function vegan_render_breadcrumbs() {
		global $post;
		...
		if ( is_page() && is_object($post) ) {
			...
			$show = vegan_get_config('show_blog_breadcrumbs', true);
			if ( !$show  ) {
				return ''; 
			}
			$breadcrumb_img = vegan_get_config('blog_breadcrumb_image');
	        $breadcrumb_color = vegan_get_config('blog_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
...
if ( ! function_exists( 'vegan_paging_nav' ) ) {
	function vegan_paging_nav() {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( '', 'vegan' ),
			'next_text' => esc_html__( '', 'vegan' ),
		) );
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'vegan' ); ?></h1>
...
if ( ! function_exists( 'vegan_post_nav' ) ) {
	function vegan_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'vegan' ); ?></h3>
			<div class="nav-links clearfix">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link','<div class="col-lg-6"><span class="meta-nav">'. esc_html__('Published In', 'vegan').'</span></div>');
				else :
					previous_post_link( '%link','<div class="pull-left"><span class="meta-nav">'. esc_html__('Previous Post', 'vegan').'</span></div>' );
					next_post_link( '%link', '<div class="pull-right"><span class="meta-nav">' . esc_html__('Next Post', 'vegan').'</span><span></span></div>');
				endif;
				?>
			</div><!-- .nav-links -->
		</nav><!-- .navigation -->
...
if ( !function_exists('vegan_pagination') ) {
    function vegan_pagination($per_page, $total, $max_num_pages = '') {
    	global $wp_query, $wp_rewrite;
        ?>
        <div class="apus-pagination">
        	<?php
        	$prev = esc_html__('Previous','vegan');
        	$next = esc_html__('Next','vegan');
        	$pages = $max_num_pages;
...
if ( !function_exists('vegan_comment_form') ) {
	function vegan_comment_form($arg, $class = 'btn-theme ') {
		global $post;
...
if (!function_exists('vegan_list_comment') ) {
	function vegan_list_comment($comment, $args, $depth) {
...
function vegan_display_footer_builder($footer) {
...
function vegan_get_blog_item_style() {
	$item_style = vegan_get_config('blog_item_style', 'grid');
	$item_style = !empty($item_style) ? $item_style : 'grid';
	return $item_style;
}

function vegan_get_blogs_layout_type() {
	$layout = vegan_get_config( 'blog_display_mode', 'grid' );
	$layout = !empty($layout) ? $layout : 'grid';
	return $layout;
}

function vegan_get_blog_thumbsize() {
	$thumbsize = vegan_get_config( 'blog_item_thumbsize', '' );
	return $thumbsize;
}
 ...
 */
function vegan_create_placeholder($size) {
...
function vegan_display_image($img) {
	if ( !empty($img) && isset($img[0]) ) {
		if (vegan_get_config('image_lazy_loading')) {
			$placeholder_image = vegan_create_placeholder(array($img[1], $img[2]));
			?>
